...
namespace Insight.Database.Structure
{
	...
	public class ListReader<T> : QueryReader<T>, IQueryReader<IList<T>>
	{
		...
		/// <inheritdoc/>
		public Task<IList<T>> ReadAsync(IDbCommand command, IDataReader reader, CancellationToken cancellationToken)
		{
#if NET35
			...
			return reader.ToListAsync(RecordReader, cancellationToken, firstRecordOnly: false)
				...
				.Unwrap()
				.ContinueWith(t => { t.Wait(); return results; }, TaskContinuationOptions.ExecuteSynchronously);
#endif
		}
	...
	public class ListReaderAdapter<TList, T1> : ListReader<T1>, IQueryReader<TList>
	{
		...
		{
			return ReadAsync(command, reader, ct)
				.ContinueWith(t => (TList)t.Result, TaskContinuationOptions.ExecuteSynchronously);
		}
		#endregion
	}
}
